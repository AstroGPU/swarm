...
class EnsembleBase {
	...
	EnsembleBase clone() {
		EnsembleBase e = createOnHostMemory(_nbod,_nsys);
		std::copy( _body.begin(), _body.end(), e._body.begin() );
		return e;
	}
	...
	static EnsembleBase createOnHostMemory(const int& nbod, const int& nsys) {
		Body* b = new Body[ element_count(nbod,nsys) ];
		return EnsembleBase(nbod,nsys,b);
	}
	static EnsembleBase createOnDeviceMemory(const int& nbod, const int& nsys) {
		Body* b ;
		cudaMalloc( &b, size_in_bytes(nbod,nsys) );
		return EnsembleBase(nbod,nsys,b);
	}

	EnsembleBase cloneToDeviceMemory() {
		EnsembleBase b = createOnDeviceMemory(_nbod,_nsys);
		copyToDeviceMemory(b);
		return b;
	}

	void copyToDeviceMemory(EnsembleBase& gpu_ens) {
		cudaMemcpy( gpu_ens.get() , get(), size_in_bytes(), cudaMemcpyHostToDevice);
	}

	void copyToHostMemory(EnsembleBase& host_ens) {
		cudaMemcpy( host_ens.get() , get(), size_in_bytes(), cudaMemcpyDeviceToHost);
	}
